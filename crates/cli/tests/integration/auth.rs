...
fn foreground_server_requires_an_explicit_admin_token_without_logging_secrets() {
    ...
    // log a bearer before failing to bind, rather than rejecting missing auth.
    let output = command
        .env_remove("TANDEM_ADMIN_TOKEN")
        .args([
            "serve",
            "--repo",
    ...
    assert!(
        stderr.contains("serve requires TANDEM_ADMIN_TOKEN"),
        "{stderr}"
    );
    assert!(
        !stderr.contains("tdma_"),
        "a generated bearer reached the logs"
    );
    assert!(!String::from_utf8_lossy(&output.stdout).contains("tdma_"));
    assert!(
        !tmp.path().join("repo").exists(),
        "missing credentials must fail before repository creation"
    );
}
...
fn head_view(addr: &str, token: &str) -> (u64, String, jj_lib::op_store::View) {
    ...
    let view_hex = jj_tandem_protocol::hex::to_hex(&op.view_id);
    ...
    let view = jj_tandem_jj::proto_convert::view_from_proto(view_proto).expect("convert the view");
...
fn fabricate(
    ...
    let view_proto = jj_tandem_jj::proto_convert::view_to_proto(view);
    let (status, view_id) = post_bytes(
        addr,
        token,
        "/api/views",
        view_proto.encode_to_vec(),
        jj_tandem_protocol::wire::HEADER_VIEW_ID,
    );
    ...
    let op_proto = jj_lib::protos::simple_op_store::Operation {
        view_id: jj_tandem_protocol::hex::from_hex(&view_id).expect("view id is hex"),
        parents: parents
            .iter()
            .map(|hex| jj_tandem_protocol::hex::from_hex(hex).expect("parent id is hex"))
            .collect(),
    ...
    let (status, op_id) = post_bytes(
        ...
        jj_tandem_protocol::wire::HEADER_OPERATION_ID,
    );
    assert_eq!(
        status, 200,
