...
//!
//! The rest of `Repository` — the RPC surface and the local jj repo — lives in the
...
pub(super) fn record_kind_for_object(kind: &str) -> Option<wal::RecordKind> {
    match kind {
        "file" => Some(wal::RecordKind::File),
        "tree" => Some(wal::RecordKind::Tree),
        "commit" => Some(wal::RecordKind::Commit),
        "symlink" => Some(wal::RecordKind::Symlink),
        _ => None,
    }
}

fn object_kind_for_record(kind: wal::RecordKind) -> Option<&'static str> {
    match kind {
        wal::RecordKind::File => Some("file"),
        wal::RecordKind::Tree => Some("tree"),
        wal::RecordKind::Commit => Some("commit"),
...
impl Repository {
    ...
    fn apply_wal_entry(&self, entry: &wal::WalEntry) -> Result<()> {
        for record in &entry.records {
            match record.kind {
                ...
                other => {
                    let kind = object_kind_for_record(other)
    ...
    fn stage_object(&self, kind: &str, id: &[u8]) -> Result<()> {
        let Some(record_kind) = record_kind_for_object(kind) else {
            return Ok(());
        };
        let data = self
