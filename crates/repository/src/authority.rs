...
//!
//! This module answers whether an authenticated workspace may publish a view.
//!
...
use super::{scope, Repository, ScopeDenied};
...
impl Repository {
    ...
    pub(super) fn check_publish_scope(
        ...
        for head_hex in &self.read_jj_op_heads()? {
            head_ids.push(
                jj_tandem_protocol::hex::from_hex(head_hex)
                    .map(OperationId::new)
