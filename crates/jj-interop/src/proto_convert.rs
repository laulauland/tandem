...
mod tests {
    ...
    fn canonical_operation_encoding_is_stable() {
        let proto = jj_lib::protos::simple_op_store::Operation {
            view_id: vec![7; 64],
            parents: vec![vec![3; 64], vec![5; 64]],
            metadata: Some(jj_lib::protos::simple_op_store::OperationMetadata {
                start_time: Some(Default::default()),
                end_time: Some(Default::default()),
        ...
        let operation = operation_from_proto(proto).expect("valid operation");
        let encoded = operation_to_proto(&operation).encode_to_vec();

        assert_eq!(encoded, expected);
    }

    #[test]
    fn malformed_operation_ids_are_rejected_at_the_boundary() {
        let proto = jj_lib::protos::simple_op_store::Operation {
            view_id: vec![0; 64],
            parents: vec![vec![0; 63]],
            ..Default::default()
        };

        assert!(operation_from_proto(proto).is_err());
    }
}
