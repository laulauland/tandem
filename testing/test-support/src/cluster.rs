...
use jj_tandem_repository::FaultPoints;
use jj_tandem_server::Server;
...
impl Cluster {
    ...
    pub fn start() -> Result<Self> {
        ...
        let mut cluster = Self {
            ...
            admin_token: jj_tandem_server::generate_admin_token(),
            runtime,
            running: None,
        };
        cluster.boot()?;
    ...
    fn boot(&mut self) -> Result<()> {
        ...
        let server = Server::from_repository(repository, &self.admin_token);

        let app = jj_tandem_server::router(Arc::new(server));
