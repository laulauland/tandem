...
use jj_tandem_test_support::readiness;
...
fn resolve_tandem_bin_path() -> PathBuf {
    ...
    // Ask Cargo for the executable it just built, respecting target overrides
    // and never silently benchmarking a stale debug binary.
    let output = Command::new("cargo")
        ...
        .output()
        .expect("build benchmark subject");
    assert!(
    ...
    String::from_utf8_lossy(&output.stdout)
        ...
        .find_map(|value| value["executable"].as_str().map(PathBuf::from))
