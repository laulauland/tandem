...
//! integration commit existing. What the two share is `Server::lock`, held only
...
use super::{HeadsMetadata, Server};
...
impl Server {
    pub(super) fn integration_metadata_path(&self) -> PathBuf {
    ...
    pub(super) fn start_integration_worker(self: &Arc<Self>) {
        if !self.integration_enabled {
            return;
        }
