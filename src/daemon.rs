...
use crate::http_client::{TandemClient, WriterClaim};
use crate::{repo_link, watch, workspace_init};
...
impl DaemonOptions {
    pub fn new(workspace_path: impl Into<PathBuf>) -> Self {
        Self {
            ...
            writer_ttl: crate::server::writer::DEFAULT_WRITER_TTL,
        }
    }
}
...
impl Daemon {
    ...
    pub fn open(settings: &UserSettings, options: &DaemonOptions) -> Result<Self> {
        ...
        let workspace = Workspace::load(
            settings,
            &workspace_path,
            &workspace_init::tandem_factories_with_defaults(),
    ...
    fn claim_writer_role(&mut self) -> bool {
        match self.client.claim_writer_role(
            &self.workspace_id,
            &self.holder,
            Some(self.writer_ttl),
        ) {
            Ok(WriterClaim::Held { .. }) => {
    ...
    pub fn run(mut self) -> Result<()> {
        ...
        spawn_event_subscription(self.server_addr().to_string(), self.client.token().to_string(), tx.clone());
...
mod tests {
    ...
    fn a_build_writing_into_an_ignored_directory_opens_no_window() {
        ...
        let ignores = GitIgnoreFile::empty()
            .chain("", Path::new("/w/.gitignore"), b"target/\nnode_modules/\n*.log\n")
        ...
        assert_eq!(
            classify_change(root, &ignores, Path::new("/w/node_modules/left-pad/index.js")),
            Change::Ignored
        );
        assert_eq!(
