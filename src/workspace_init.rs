...
use crate::{backend, op_heads_store, op_store, proto_convert};
...
fn published_workspace_head(
    ...
    for head in &heads.heads {
        ...
        let operation =
            proto_convert::operation_from_proto(operation_proto).context("cannot read operation")?;
...
///
/// The rule itself is [`crate::placeholder`]; this is the client's way of
...
fn is_fresh_workspace_placeholder(client: &TandemClient, commit_id: &CommitId) -> Result<bool> {
    ...
    let data = client
        .get_object(crate::wire::KIND_COMMIT, commit_id.as_bytes())
    ...
    Ok(crate::placeholder::is_fresh_workspace_placeholder(
        &commit.description,
        &parents,
        &root_tree,
        &info.root_commit_id,
...
pub fn init_tandem_workspace(
    ...
    let (path, _origin) = clone_tandem_workspace(
        settings,
        server_addr,
        token,
        workspace_name,
