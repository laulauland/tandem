...
//! (`crate::server::writer`), which is a separate thing entirely: a token says
...
impl Authority {
    ...
    /// The workspace this authority is confined to, if it is confined at all.
    pub fn workspace(&self) -> Option<&str> {
        match self {
            Authority::Admin => None,
            Authority::Workspace(name) => Some(name),
        }
    }
...
/// The bookmark namespace a workspace owns.
///
/// Invariant 3 of the design doc: an agent writes `agent-a/task-42`, and
/// `main` moves only by an integrator. The namespace is the workspace name and
/// a slash — everything else in the view is somebody else's.
pub fn namespace_prefix(workspace_id: &str) -> String {
    format!("{workspace_id}/")
}
